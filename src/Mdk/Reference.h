...
{
template <typename T>
class Reference : public ::Smp::Mdk::Object,
...
public:
    ...
    virtual T *At(
        ::Smp::Int32 index) const
    {
        T *ret = NULL;

        if ((index >= 0) && (index < this->m_providers.size()))
        {
            ret = this->m_providers[index];
        }

        return ret;
    }

    virtual T *operator[](::Smp::Int32 index)
    {
        return At(index);
    }
